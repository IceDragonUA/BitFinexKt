...
class SyncDetailWorkHelper @Inject constructor(private val context: Context) : SyncScheduler {

    override fun startSchedule(period: Long, timeUnit: TimeUnit) {
        val constraints = Constraints.Builder()
                .setRequiredNetworkType(NetworkType.CONNECTED)
                .build()
        val updateMenuWork = PeriodicWorkRequest.Builder(UpdateDetailWorker::class.java, period, timeUnit)
    ...
    override fun startUpdate() {
        val updateMenuWork = OneTimeWorkRequest
                .Builder(UpdateDetailWorker::class.java)
                .addTag(UPDATE_APP_WORK)
                .build()
